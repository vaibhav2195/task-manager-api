...
document.addEventListener('DOMContentLoaded', () => {
    // Dynamic API Host Resolution for Vercel & Render & Local Dev
    const API_HOST = (window.location.hostname === 'localhost' || window.location.hostname === '127.0.0.1')
        ? 'http://localhost:8000'
        : (window.location.hostname.includes('onrender.com') 
            ? '' 
            : 'https://task-manager-api-l6ne.onrender.com');

    const API_BASE = `${API_HOST}/tasks`;
    const HEALTH_URL = `${API_HOST}/health`;
    ...
    function updateMetrics() {
        ...
        if (metricPending) metricPending.textContent = active;
        if (metricRate) metricRate.textContent = `${rate}%`;
        if (metricHigh) metricHigh.textContent = highPriority;
    ...
    document.querySelectorAll('.nav-item').forEach(btn => {
        btn.addEventListener('click', () => {
            ...
            if (sectionTitle) sectionTitle.textContent = btn.innerText.split('\n')[0].trim();
            renderTaskList();
        });
    });
